...
function App() {
  ...
  const getData = () => {
    return async (e: any) => {
      ...
      await ffmpeg.createDir('parts');
      let $output_path_format = `parts/part_%d.${input_ext}`;

      console.log($output_path_format);
      console.log(video);

      await ffmpeg.exec(["-y", "-i", video, "-f", "segment", "-segment_time", "5", "-vcodec", "copy", "-reset_timestamps", "1", "-map", "0:v", $output_path_format]);


      // get files from /temporary
      let file_data = await ffmpeg.listDir('/parts');
      console.log(file_data);

      let promeises = new Array();
      for (let i = 0; i < file_data.length; i++) {
        if (file_data[i].isDir) continue;
        console.log(file_data[i].name);
        promeises.push(ffmpegRef.current.exec(['-i', `/parts/${file_data[i].name}`, '-c:v', 'libx264', '-crf', '30', `/parts/compressed-part-${i - 2}.mp4`]));
        // await ffmpeg.deleteFile(`/parts/${file_data[i].name}`);

      }

      await Promise.all(promeises);

      
      file_data = await ffmpeg.listDir('/parts');
      console.log(file_data);

      // create parts.text conatain all the part path 
      let text = '';
      for (let i = 0; i < file_data.length; i++) {
        if (file_data[i].isDir) continue;
        text += `file '/parts/compressed-part-${i - 2}.mp4'\n`;
      }

      console.log(text);

      await ffmpeg.writeFile('parts.txt', text);
      await ffmpeg.exec(["-f", "concat", "-safe", "0", "-y", "-i", `parts.txt`, "-c", "copy", "output.mp4"]);


      let root = await ffmpeg.listDir('/');
      let parts = await ffmpeg.listDir('/parts');
      console.log(root, parts);

      const fileD_ata = await ffmpeg.readFile('output.mp4');
      const data = new Uint8Array(fileD_ata as ArrayBuffer);
      ...
      setResultMeta(getMetaData(null, data));

      // clear all the files
      await ffmpeg.deleteFile(video);
      root = await ffmpeg.listDir('/');
      console.log(root);
      await ffmpeg.deleteFile('output.mp4');
      root = await ffmpeg.listDir('/');
      console.log(root);
      await ffmpeg.deleteFile('parts.txt');
      root = await ffmpeg.listDir('/parts');

      for (let i = 0; i < file_data.length; i++) {
        if (file_data[i].isDir) continue;
        await ffmpeg.deleteFile(`/parts/compressed-part-${i - 2}.mp4`);
      }

      root = await ffmpeg.listDir('/parts');
      console.log(root);
      await ffmpeg.deleteDir('parts');
